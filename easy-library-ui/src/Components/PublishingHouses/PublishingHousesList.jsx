...
import PublishingHouseModal from './PublishingHouseModal';
import PublishingHouseListItem from './PublishingHouseListItem';
import PublishingHousesService from '../../Services/publishingHouses.service';
import '../../utils/alert.css';
import { exportToExcel } from '../../utils/exportToExcel';

const PublishingHousesList = () => {
    ...
    const [currentPublishingHouse, setCurrentPublishingHouse] = useState(null);

    const fetchPublishingHouses = async () => {
        try {
            const publishingHousesList = await PublishingHousesService.getAllPublishingHouses();
            setPublishingHouses(publishingHousesList);
        } catch (error) {
            setErrorMessage(error.message);
            setErrorPrompt('Ошибка загрузки спискав издательст.');
        } finally {
            setLoading(false);
        }
    };

    const handleSave = async (publishingHouse) => {
        if (publishingHouse.id) {
            await PublishingHousesService.updatePublishingHouse(publishingHouse);
        } else {
            await PublishingHousesService.createPublishingHouse(publishingHouse);
        }
        await fetchPublishingHouses();
        setCurrentPublishingHouse(null);
    };
    ...
    const handleEdit = async (publishingHouse) => {
        setCurrentPublishingHouse(publishingHouse);
        setShowModal(true);
    };
    ...
    const handleDelete = async (id) => {
        await PublishingHousesService.deletePublishingHouse(id);
        await fetchPublishingHouses();
    }
    ...
    const handleExport = async () => {
        try {
            ...
            const exportData = publishingHouses.map(publishingHouse => ({
                'Наименование': publishingHouse.name
            }));
            exportToExcel(exportData, 'PublishingHouses');
        }
        catch (error) {
            console.error(`Failed to export publishing houses: ${error}`)
        }
    };
    ...
    return (
        <div className="container py-5" style={{ 'minHeight': '60vh' }}>
                ...
                <div className="d-flex justify-content-between align-items-center w-100 my-2">
                    ...
                    <div className='d-flex'>
                        <button className="btn text-white btn-outline-secondary me-2 d-flex align-items-center p-2" onClick={handleAdd}>
                            <span className='ms-1'>Добавить издательство</span>
                        </button>
            ...
            <PublishingHouseModal
                show={showModal}
                handleClose={handleClose}
                publishingHouse={currentPublishingHouse}
