...
import BookAuthorModal from './BookAuthorsModal'
import BookAuthorListItem from './BookAuthorListItem'
...
const BookAuthorsList = () => {
    ...
    const [currentBookAuthor, setCurrentBookAuthor] = useState(null);

    const fetchBookAuthors = async () => {
        try {
            const bookAuthorsList = await BookAuthorsService.getAllBookAuthors('display');
            setBookAuthors(bookAuthorsList);
        } catch (error) {
    ...
    const fetchBookAuthor = async (id) => {
        try {
            return await BookAuthorsService.getById(id);
        } catch (error) {
            setErrorMessage(error);
            setErrorPrompt('Не удалось получить данные автора.');
        }
    };

    const handleSave = async (bookAuthor) => {
        if (bookAuthor.id) {
            await BookAuthorsService.updateBookAuthor(bookAuthor);
        } else {
            await BookAuthorsService.createBookAuthor(bookAuthor);
        }
    ...
    const handleEdit = async (id) => {
        const fullBookAuthor = await fetchBookAuthor(id);
        setCurrentBookAuthor(fullBookAuthor);
        setShowModal(true);
    };
    ...
    const handleDelete = async (id) => {
        await BookAuthorsService.deleteBookAuthor(id);
        await fetchBookAuthors();
    }
    ...
    const handleExport = async () => {
        try {
            const fullBookAuthors = await BookAuthorsService.getAllBookAuthors('full');
            const exportData = fullBookAuthors.map(bookAuthor => ({
                'Имя': bookAuthor.name,
                'Описание': bookAuthor.bio,
            }));
            exportToExcel(exportData, 'BookAuthors');
        }
        catch (error) {
            console.error(`Failed to export book authors: ${error}`)
        }
    };
                ...
                <div className="d-flex justify-content-between align-items-center w-100 my-2">
                    ...
                    <div className='d-flex'>
                        <button className="btn text-white btn-outline-secondary me-2 d-flex align-items-center p-2" onClick={handleAdd}>
                            <span className='ms-1'>Добавить автора</span>
                        </button>
